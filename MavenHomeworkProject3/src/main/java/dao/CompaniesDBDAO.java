...
public class CompaniesDBDAO implements CompaniesDAO {
	...
	public boolean isCompanyExists(String email, String password) throws SQLException {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT company_email, password FROM companies");
	...
	public void updateCompany(Company company) throws SQLException {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("UPDATE companies SET company_name = ?, company_email = ?, password = ? WHERE id = ?");
			ps.setString(1, company.getName());
			ps.setString(2, company.getEmail());
			ps.setString(3, company.getPassword());
